...
1. **新建任务页**（`/`）——输入主题、上传参考资料、选模板、设页数、确认模板比例并开始生成。
...
结构自上而下：标题区 → 主卡片（演示主题 textarea，0/200 计数，4 个示例主题胶囊）→ 参考资料上传区（可选，PDF/Word/Excel，dropzone）→ 模板选择行（横向卡片，展示最近 4 个 + "浏览模板库 →"链接）→ 生成设置（页数 Slider 5–30 默认 12；只读展示模板自带的 16:9 / 4:3 比例）→ 底部"✦ 开始生成"主按钮。
...
- 网格卡片（4 列）：缩略图（解析完成前用模板主色画的占位封面）、名称、主要配色色板、"N 页 · 比例 · N 种页面形式"、页面形式标签、状态徽章；详情弹窗展示完整页面形式和带色值的主色板。
- **解析中**：卡片封面模糊 + "解析中 62%" + 细进度条；进度来自 SSE `template.parse_progress`。
- **解析失败**：显示简短原因（如"文件包含不支持的母版结构"）+ "重新解析"按钮。
- 上传：点击或拖入 `.pptx` → `POST /api/templates`（multipart）→ 新卡片以"解析中"插入网格首位。
...
主题、附件、选中模板、页数、模板比例；模板 ID 与比例必须原子更新，`createTask()` 返回 task_id。
